...
export default function ProfileCard() {
  ...
  // Fetch profiles assigned to the coach
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {profiles.length > 0 ? (
            profiles.map((profile) => (
              <div
                key={profile._id}
                className={`bg-gray-800/50 w-72 border border-gray-700 rounded-lg overflow-hidden hover:bg-gray-700/40 transition-all duration-300 ${
                  expandedCards[profile._id] ? 'h-auto' : 'h-[200px]'
                }`}
              >
                <div className="p-6">
                  ...
                  {/* Expanded details */}
                  <div
                    className={`mt-4 space-y-2 text-gray-400 transition-all duration-300 ${
                      expandedCards[profile._id] ? 'block' : 'hidden'
                    }`}
                  >
                  ...
                  >
                    ...
                    <ChevronDown
                      className={`w-4 h-auto  transform transition-transform ${
